...
def _write_fixture(root: Path) -> dict[str, str]:
    ...
    (repo / "web" / "handlers.js").write_text(
        "export function importedExport() {\n"
        "  fetch('/api/v1/assets/export', {method: 'POST'});\n"
        "}\n",
        encoding="utf-8",
    )
...
def run_calibration() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="federation-audit-calibration-") as temp:
        ...
        manifest = {"repositories": [{"id": "calibration", "repository": "fixture/calibration", "commit": "a" * 40, "workspace_directory": "calibration"}]}
        result = strict_scan_federation(root, manifest)
        by_label = {item["surface"]["label"]: item["classification"] for item in result["traces"] if item["surface"]["kind"] == "gui-control"}
        tp = fp = fn = tn = 0
        cases: list[dict[str, str | bool]] = []
        for label, expected_class in expected.items():
            actual = by_label.get(label, "MISSING_FROM_SCAN")
            expected_positive, actual_positive = expected_class == POSITIVE, actual == POSITIVE
            if expected_positive and actual_positive: tp += 1
            elif not expected_positive and actual_positive: fp += 1
            elif expected_positive and not actual_positive: fn += 1
            else: tn += 1
            cases.append({"label": label, "expected": expected_class, "actual": actual, "passed": actual == expected_class})
        declaration_violations = [item for item in result["traces"] if item["surface"]["kind"] in {"route", "command", "workflow-stage"} and item["classification"] == POSITIVE]
        ...
        return {"true_positive": tp, "true_negative": tn, "false_positive": fp, "false_negative": fn, "precision": round(precision, 6), "recall": round(recall, 6), "cases": cases, "declaration_promotion_violations": len(declaration_violations), "passed": passed}
