...
def classify_observations(observations: dict[str, object]) -> tuple[Classification, float, str | None, str]:
    ...
    if observations.get("unsafe_boundary"):
        return (
            Classification.UNSAFE_TO_PROBE,
            0.98,
            "side-effect-boundary",
            "Add an emulator or approved interceptor.",
        )
    if observations.get("runtime_failure"):
        return (
            ...
            0.98,
            "runtime",
            "Repair the recorded exception before re-probing.",
        )
    if observations.get("placeholder"):
        return (
            ...
            0.97,
            "implementation",
            "Replace the stub with a contract-backed implementation.",
        )
    if observations.get("unreachable"):
        return (
            Classification.UNREACHABLE,
            0.95,
            "navigation-or-dispatch",
            "Add a normal route, command, or workflow edge.",
        )
    if observations.get("undeclared_precondition"):
        return (
            Classification.PRECONDITION_UNDECLARED,
            0.94,
            "precondition",
            "Declare and validate the hidden precondition.",
        )
    if observations.get("contract_mismatch"):
        return (
            ...
            "Align method, path, and payload schema.",
        )
    if observations.get("target_missing") or (
        ...
        return (
            ...
            0.99,
            "dispatch-target",
            "Implement the target or correct the binding.",
        )
    if observations.get("handler_bound") is False:
        return (
            Classification.UI_NO_OP,
            0.99,
            "gui-to-handler",
            "Bind a meaningful handler or render a non-action control.",
        )
    if observations.get("blocked_precondition"):
        return (
            Classification.WIRED_BUT_BLOCKED,
            0.95,
            "dependency-or-precondition",
            "Satisfy or explicitly surface the blocking condition.",
        )
    ...
    if (
        ...
        return (
            Classification.EXECUTABLE_CONFIRMED,
            0.99,
            None,
            "Retain the T1/T2 receipt as a regression gate.",
        )
    ...
    if (
        ...
        and (observations.get("side_effect_intercepted") or observations.get("target_resolution_evidence"))
    ):
        return (
            Classification.EXECUTABLE_BY_CONTRACT,
            0.97,
            None,
            "Graduate to an isolated shadow runtime for confirmation.",
        )
    ...
    if observations.get("handler_bound") and (
        ...
        return (
            ...
            0.88,
            "handler-to-terminal",
            "Complete and prove the downstream target edge.",
        )
