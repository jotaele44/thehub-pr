...
def _run_boot(
    command: list[str], cwd: Path, env: dict[str, str], startup_seconds: int
) -> tuple[int | None, str, str, bool]:
    try:
        proc = subprocess.Popen(
            command, cwd=cwd, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
        )
...
def execute_probe(workspace_root: Path, shadow_root: Path, probe: Probe) -> dict[str, Any]:
    ...
    probe_shadow, home, tmp, fs_sink = (
        shadow_root / probe.probe_id,
        shadow_root / probe.probe_id / "home",
        shadow_root / probe.probe_id / "tmp",
        shadow_root / probe.probe_id / "fs",
    )
    ...
    timed_out = False
    if probe.mode == "boot":
        returncode, stdout, stderr, alive_after_startup = _run_boot(
            list(probe.command), cwd, env, probe.startup_seconds
        )
    ...
    if execution_ok and probe.mode == "command" and (probe.expected_stdout or probe.expected_artifact):
        gate = "G6"
    minimum_met, passed = (
        GATE_ORDER[gate] >= GATE_ORDER[probe.minimum_gate],
        execution_ok and GATE_ORDER[gate] >= GATE_ORDER[probe.minimum_gate],
    )
    blocked_attempts = (
        len(blocked_log.read_text(encoding="utf-8").splitlines()) if blocked_log.is_file() else 0
    )
    receipt = {
...
def runtime_certify(
    ...
    summary = {
        ...
        "entry_points_present": sum(
            sum(bool(ep["exists"]) for ep in item["entry_points"]) for item in workspace_receipts
        ),
        "probes_expected": len(probes),
        "probes_passed": sum(bool(item.get("passed")) for item in probe_receipts),
        "g6_receipts": sum(bool(item.get("gate") == "G6" and item.get("passed")) for item in probe_receipts),
        "production_egress_events": sum(item.get("production_egress") is True for item in probe_receipts),
        "production_credential_events": sum(
            item.get("production_credentials") is True for item in probe_receipts
        ),
        "uncontained_side_effects": sum(
            int(item.get("uncontained_side_effects") or 0) for item in probe_receipts
        ),
