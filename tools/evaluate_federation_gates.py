...
GATE_RULES = [
    GateRule(
        ...
        deferred_reason=(
            ...
            "cbab79c; the drift is adjudicated rather than ignored, which is what this gate's "
            "second clause provides for. src/hub/cli.py is byte-identical across 58a159f, "
            "e668cad, cbab79c, and this branch, so every operations-policy provenance pin "
            "reading cli.py@58a159ff still names the exact content it was derived from. This "
            "stack modifies no file under src/hub. Not receipt-derivable, because no operation "
            "observes git state."
        ),
    ),
    GateRule(
