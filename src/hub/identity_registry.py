...
class IdentityRegistry:
    ...
    def resolve_member(
        ...
        with self._connect() as db:
            row = db.execute(
                "SELECT federation_entity_id FROM federation_entity_members WHERE source_producer=? AND local_record_id=? AND status='ACTIVE'",
    ...
    def create_entity(
        ...
        *,
        ...
        with self._connect() as db:
            row = db.execute(
                "SELECT entity_type,canonical_name,federation_authority FROM federation_entities WHERE federation_entity_id=?",
                (fed_id,),
            ).fetchone()
            if row:
                ...
                return fed_id
            db.execute(
                "INSERT INTO federation_entities (federation_entity_id,entity_type,canonical_name,domain_owner,federation_authority,created_at,updated_at) VALUES (?,?,?,?,?,?,?)",
                (
                    fed_id,
                    entity_type,
                    canonical_name,
    ...
    def record_resolution_decision(
        ...
        *,
        ...
        with self._connect() as db:
            row = db.execute(
                "SELECT decision_type,outcome,reason_code,evidence_json,candidate_ref,supersedes_decision_id,superseded_by,decided_by FROM federation_resolution_decisions WHERE decision_id=?",
                (decision_id,),
            ).fetchone()
            if row:
                if tuple(row) != expected:
                    raise ValueError(
                        "immutable decision_id already recorded with different semantics"
                    )
                return
            db.execute(
                "INSERT INTO federation_resolution_decisions (decision_id,decision_type,outcome,reason_code,evidence_json,candidate_ref,supersedes_decision_id,superseded_by,decided_by,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
                (decision_id, *expected, _utcnow()),
            )
    ...
    def attach_member(
        ...
        *,
        ...
        if match_class not in ALLOWED_MATCH_CLASSES:
            raise ValueError(
                f"non-adjudicative match class cannot merge: {match_class}"
            )
        digest = payload_hash(payload)
        with self._connect() as db:
            decision = db.execute(
                "SELECT decision_type,outcome FROM federation_resolution_decisions WHERE decision_id=?",
                (decision_id,),
            ).fetchone()
            if not decision or decision["decision_type"] != "entity_identity_decision":
                raise ValueError(
                    "membership requires entity_resolution.v1 identity decision"
                )
            if decision["outcome"] != "MERGE":
                raise ValueError("membership requires MERGE outcome")
            entity = db.execute(
                "SELECT status FROM federation_entities WHERE federation_entity_id=?",
                (federation_entity_id,),
            ).fetchone()
            if not entity or entity["status"] != "ACTIVE":
                raise ValueError("membership target must be active")
            row = db.execute(
                "SELECT federation_entity_id,source_revision,source_sequence,payload_hash,decision_id FROM federation_entity_members WHERE source_producer=? AND local_record_id=?",
                (source_producer, local_record_id),
            ).fetchone()
            expected = (
                federation_entity_id,
            ...
            if row:
                ...
                if row["federation_entity_id"] != federation_entity_id:
                    raise ValueError(
                        "producer member already belongs to a different federation entity"
                    )
                raise ValueError(
                    "member mutation requires an explicit supersession event"
                )
            db.execute(
                "INSERT INTO federation_entity_members (source_producer,local_record_id,federation_entity_id,source_revision,source_sequence,payload_hash,match_class,decision_id,created_at) VALUES (?,?,?,?,?,?,?,?,?)",
                (
                    source_producer,
                    local_record_id,
                    federation_entity_id,
    ...
    def add_alias(
        ...
        *,
        ...
        with self._connect() as db:
            row = db.execute(
                "SELECT alias_id FROM federation_aliases WHERE federation_entity_id=? AND normalized_alias=? AND status='ACTIVE'",
                (federation_entity_id, normalized),
            ).fetchone()
            if row:
                return str(row[0])
            db.execute(
                "INSERT INTO federation_aliases (alias_id,federation_entity_id,alias_text,normalized_alias,provenance_id,decision_id,created_at) VALUES (?,?,?,?,?,?,?)",
                (
                    alias_id,
                    federation_entity_id,
                    alias_text,
    ...
    def add_identifier(
        ...
        *,
        ...
        with self._connect() as db:
            row = db.execute(
                "SELECT identifier_id,federation_entity_id FROM federation_identifiers WHERE namespace=? AND identifier_value=? AND status='ACTIVE'",
                (namespace, identifier_value),
            ).fetchone()
            if row:
                if row["federation_entity_id"] != federation_entity_id:
                    raise ValueError(
                        "global identifier collision cannot auto-merge entities"
                    )
                return str(row["identifier_id"])
            db.execute(
                "INSERT INTO federation_identifiers (identifier_id,federation_entity_id,namespace,identifier_value,provenance_id,decision_id,created_at) VALUES (?,?,?,?,?,?,?)",
                (
                    identifier_id,
                    federation_entity_id,
                    namespace,
    ...
    def add_relationship(
        self,
        *,
        source_federation_entity_id: str,
        target_federation_entity_id: str,
        ...
        relationship_id = stable_id(
            "frel",
            source_federation_entity_id,
            relationship_type,
            target_federation_entity_id,
            domain_owner or "",
        )
        now = _utcnow()
        with self._connect() as db:
            ...
            db.execute(
                "INSERT OR IGNORE INTO federation_relationships (federation_relationship_id,source_federation_entity_id,target_federation_entity_id,relationship_type,domain_owner,federation_authority,confidence,provenance_json,decision_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                (
                    relationship_id,
                    source_federation_entity_id,
                    target_federation_entity_id,
                    ...
                    FEDERATION_AUTHORITY,
                    confidence,
                    json.dumps(sorted(set(provenance_ids))),
                    decision_id,
                    now,
                    now,
                ),
    ...
    def register_revision_contract(
        self, source_producer: str, revision_mode: str
    ) -> None:
        if revision_mode not in REVISION_MODES:
            raise ValueError("unsupported revision ordering mode")
        with self._connect() as db:
            row = db.execute(
                "SELECT revision_mode FROM federation_producer_revision_contracts WHERE source_producer=?",
                (source_producer,),
            ).fetchone()
            if row and row["revision_mode"] != revision_mode:
                raise ValueError("revision contract is immutable once recorded")
            db.execute(
                "INSERT OR IGNORE INTO federation_producer_revision_contracts (source_producer,revision_mode,created_at) VALUES (?,?,?)",
                (source_producer, revision_mode, _utcnow()),
            )

    def _event_order_disposition(
        self,
        ...
        prior = db.execute(
            "SELECT source_revision,source_sequence FROM federation_events WHERE source_producer=? AND local_record_id=? AND disposition='APPLIED' ORDER BY created_at DESC LIMIT 1",
            (source_producer, local_record_id),
        ).fetchone()
            ...
            if prior is not None and prior["source_sequence"] is not None:
                ...
                if source_sequence == previous:
                    return (
                        ...
                        "source_sequence reused by non-identical event",
                    )
            return None, None
        ...
        if prior is not None and prior["source_revision"] is not None:
            ...
            if current == previous:
                return (
                    "REJECTED_OUT_OF_ORDER",
                    "source_revision reused by non-identical event",
                )
        return None, None
    ...
    def record_event(
        ...
        *,
        ...
        payload_hash_value: str | None = None,
        authority: str = FEDERATION_AUTHORITY,
        apply_state_mutation: bool = False,
    ) -> tuple[str, str]:
        ...
        with self._connect() as db:
            ...
            existing = db.execute(
                "SELECT payload_hash,payload_json FROM federation_events WHERE event_id=?",
                (event_id,),
            ).fetchone()
            if existing:
                if existing["payload_hash"] != supplied_hash or existing[
                    "payload_json"
                ] != _canonical_json(payload):
                    raise ValueError("event_id collision")
                db.execute(
                    "INSERT INTO federation_event_attempts (event_id,disposition,rejection_reason,attempted_at) VALUES (?,?,?,?)",
                    (event_id, "IDEMPOTENT_REPLAY", None, _utcnow()),
                )
                db.commit()
            ...
            if schema_version != SCHEMA_VERSION:
                ...
                disposition, rejection_reason = (
                    "REJECTED_AUTHORITY",
                    "federation authority mismatch",
                )
            elif event_type not in ALLOWED_EVENTS:
                disposition, rejection_reason = (
                    "REJECTED_INVARIANT",
                    "unsupported federation event",
                )
            else:
                rejected, reason = self._event_order_disposition(
                    db,
                    source_producer,
                    local_record_id,
                    source_revision,
            ...
            db.execute(
                "INSERT INTO federation_events (event_id,event_type,schema_version,source_producer,local_record_id,source_revision,source_sequence,payload_hash,payload_json,effective_at,disposition,rejection_reason,state_mutated,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    event_id,
                    event_type,
                    schema_version,
                    ...
                    disposition,
                    rejection_reason,
                    1 if disposition == "APPLIED" and apply_state_mutation else 0,
                    _utcnow(),
                ),
            )
            db.execute(
                "INSERT INTO federation_event_attempts (event_id,disposition,rejection_reason,attempted_at) VALUES (?,?,?,?)",
                (event_id, disposition, rejection_reason, _utcnow()),
            )
            db.commit()
        return event_id, disposition

    def _entity_state_hash(
        self, db: sqlite3.Connection, entity_ids: Sequence[str]
    ) -> str:
        payload: dict[str, Any] = {}
        for table in (
            "federation_entities",
            "federation_entity_members",
            "federation_aliases",
            "federation_identifiers",
        ):
            ph = ",".join("?" for _ in entity_ids)
            rows = db.execute(
                f"SELECT * FROM {table} WHERE federation_entity_id IN ({ph}) ORDER BY federation_entity_id",
                tuple(entity_ids),
            ).fetchall()
            payload[table] = [dict(row) for row in rows]
        ph = ",".join("?" for _ in entity_ids)
        rows = db.execute(
            f"SELECT * FROM federation_relationships WHERE source_federation_entity_id IN ({ph}) OR target_federation_entity_id IN ({ph}) ORDER BY federation_relationship_id",
    ...
    def merge_entities(
        self,
        *,
        from_federation_entity_id: str,
        to_federation_entity_id: str,
        decision_id: str,
    ) -> str:
        ...
        with self._connect() as db:
            db.execute("BEGIN IMMEDIATE")
            history = db.execute(
                "SELECT * FROM federation_merge_history WHERE from_federation_entity_id=?",
                (from_federation_entity_id,),
            ).fetchone()
            if history:
                if (
                    history["to_federation_entity_id"] == to_federation_entity_id
                    and history["decision_id"] == decision_id
                ):
                    db.commit()
                    return str(history["merge_id"])
                db.rollback()
                raise ValueError(
                    "incompatible second merge rejected; history is immutable"
                )
            decision = db.execute(
                "SELECT decision_type,outcome,reason_code,evidence_json FROM federation_resolution_decisions WHERE decision_id=?",
                (decision_id,),
            ).fetchone()
            if (
                not decision
                or decision["decision_type"] != "entity_identity_decision"
                or decision["outcome"] != "MERGE"
            ):
                db.rollback()
            ...
            before_hash = self._entity_state_hash(
                db, [from_federation_entity_id, to_federation_entity_id]
            )
            now = _utcnow()
            db.execute(
                "UPDATE federation_entity_members SET federation_entity_id=? WHERE federation_entity_id=? AND status='ACTIVE'",
                (to_federation_entity_id, from_federation_entity_id),
            )
            for alias in db.execute(
                "SELECT alias_id,normalized_alias FROM federation_aliases WHERE federation_entity_id=? AND status='ACTIVE'",
                (from_federation_entity_id,),
            ).fetchall():
                duplicate = db.execute(
                    "SELECT alias_id FROM federation_aliases WHERE federation_entity_id=? AND normalized_alias=? AND status='ACTIVE'",
                    (to_federation_entity_id, alias["normalized_alias"]),
                ).fetchone()
                if duplicate:
                    db.execute(
                        "UPDATE federation_aliases SET status='SUPERSEDED',superseded_by_alias_id=? WHERE alias_id=?",
                        (duplicate["alias_id"], alias["alias_id"]),
                    )
                else:
                    db.execute(
                        "UPDATE federation_aliases SET federation_entity_id=? WHERE alias_id=?",
                        (to_federation_entity_id, alias["alias_id"]),
                    )
            db.execute(
                "UPDATE federation_identifiers SET federation_entity_id=? WHERE federation_entity_id=? AND status='ACTIVE'",
                (to_federation_entity_id, from_federation_entity_id),
            )
            rels = db.execute(
                "SELECT federation_relationship_id,source_federation_entity_id,target_federation_entity_id,relationship_type,domain_owner FROM federation_relationships WHERE status='ACTIVE' AND (source_federation_entity_id=? OR target_federation_entity_id=?) ORDER BY federation_relationship_id",
                (from_federation_entity_id, from_federation_entity_id),
            ).fetchall()
            for rel in rels:
                new_source = (
                    to_federation_entity_id
                    if rel["source_federation_entity_id"] == from_federation_entity_id
                    else rel["source_federation_entity_id"]
                )
                new_target = (
                    to_federation_entity_id
                    if rel["target_federation_entity_id"] == from_federation_entity_id
                    else rel["target_federation_entity_id"]
                )
                duplicate = db.execute(
                    "SELECT federation_relationship_id FROM federation_relationships WHERE status='ACTIVE' AND federation_relationship_id<>? AND source_federation_entity_id=? AND target_federation_entity_id=? AND relationship_type=? AND COALESCE(domain_owner,'')=COALESCE(?, '') ORDER BY federation_relationship_id LIMIT 1",
                    (
                        rel["federation_relationship_id"],
                        new_source,
                        new_target,
                ...
                if duplicate:
                    db.execute(
                        "UPDATE federation_relationships SET status='SUPERSEDED',superseded_by=?,valid_to=?,updated_at=? WHERE federation_relationship_id=?",
                        (
                            duplicate["federation_relationship_id"],
                            now,
                            now,
                            rel["federation_relationship_id"],
                        ),
                    ...
                    db.execute(
                        "UPDATE federation_relationships SET source_federation_entity_id=?,target_federation_entity_id=?,updated_at=? WHERE federation_relationship_id=?",
                        (
                            new_source,
                            new_target,
                            now,
            ...
            db.execute(
                "UPDATE federation_entities SET status='SUPERSEDED',superseded_by=?,valid_to=?,updated_at=? WHERE federation_entity_id=?",
                (to_federation_entity_id, now, now, from_federation_entity_id),
            )
            after_hash = self._entity_state_hash(
                db, [from_federation_entity_id, to_federation_entity_id]
            )
            merge_id = stable_id(
                "fmerge",
            ...
            db.execute(
                "INSERT INTO federation_merge_history (merge_id,from_federation_entity_id,to_federation_entity_id,decision_id,reason_code,evidence_json,before_hash,after_hash,created_at) VALUES (?,?,?,?,?,?,?,?,?)",
                (
                    merge_id,
                    from_federation_entity_id,
                    to_federation_entity_id,
    ...
    def supersede_entity(
        self, *, federation_entity_id: str, superseded_by: str, decision_id: str
    ) -> None:
        with self._connect() as db:
            db.execute("BEGIN IMMEDIATE")
            decision = db.execute(
                "SELECT decision_type FROM federation_resolution_decisions WHERE decision_id=?",
                (decision_id,),
            ).fetchone()
            if not decision or decision["decision_type"] != "superseded_decision":
                db.rollback()
                raise ValueError(
                    "supersede requires entity_resolution.v1 superseded_decision"
                )
            target = db.execute(
                "SELECT status FROM federation_entities WHERE federation_entity_id=?",
                (superseded_by,),
            ...
            db.execute(
                "UPDATE federation_entities SET status='SUPERSEDED',superseded_by=?,valid_to=?,updated_at=? WHERE federation_entity_id=?",
                (superseded_by, now, now, federation_entity_id),
            )
            db.commit()
    ...
    def tombstone_entity(self, federation_entity_id: str) -> None:
        with self._connect() as db:
            ...
            now = _utcnow()
            db.execute(
                "UPDATE federation_entities SET status='TOMBSTONED',valid_to=?,updated_at=? WHERE federation_entity_id=?",
                (now, now, federation_entity_id),
            )
            db.execute(
                "UPDATE federation_entity_members SET status='TOMBSTONED',valid_to=? WHERE federation_entity_id=? AND status='ACTIVE'",
                (now, federation_entity_id),
            )
            db.execute(
                "UPDATE federation_aliases SET status='TOMBSTONED' WHERE federation_entity_id=? AND status='ACTIVE'",
                (federation_entity_id,),
            )
            db.execute(
                "UPDATE federation_identifiers SET status='TOMBSTONED' WHERE federation_entity_id=? AND status='ACTIVE'",
                (federation_entity_id,),
            )
            db.execute(
                "UPDATE federation_relationships SET status='TOMBSTONED',valid_to=?,updated_at=? WHERE status='ACTIVE' AND (source_federation_entity_id=? OR target_federation_entity_id=?)",
                (now, now, federation_entity_id, federation_entity_id),
            )
            db.commit()
    ...
    def integrity_report(self) -> dict[str, int]:
        ...
        with self._connect() as db:
            return {
                name: int(db.execute(sql).fetchone()[0]) for name, sql in checks.items()
            }
