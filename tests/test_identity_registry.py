...
def test_alias_collision_does_not_automerge(tmp_path):
    ...
    prov = reg.add_provenance(
        source_producer="moneysweep-pr", local_record_id="x", evidence_id="e1"
    )
    reg.add_alias(
        federation_entity_id=a,
        alias_text="Same Name",
        provenance_id=prov,
        decision_id="decision-a",
    )
    reg.add_alias(
        federation_entity_id=b,
        alias_text="Same Name",
        provenance_id=prov,
        decision_id="decision-a",
    )
...
def test_transactional_merge_moves_members_and_redirects_edges(tmp_path):
    ...
    reg.attach_member(
        federation_entity_id=a,
        source_producer="moneysweep-pr",
        local_record_id="ent_a",
        source_revision="1",
        payload={"id": "a"},
        match_class="EXACT_IDENTIFIER",
        decision_id="decision-merge",
    )
    rel = reg.add_relationship(
        source_federation_entity_id=a,
        target_federation_entity_id=c,
        relationship_type="operates",
        provenance_ids=["p1"],
    )
    merge_id = reg.merge_entities(
        from_federation_entity_id=a,
    ...
    with sqlite3.connect(tmp_path / "hub.db") as db:
        ...
        edge = db.execute(
            "SELECT source_federation_entity_id,status FROM federation_relationships WHERE federation_relationship_id=?",
            (rel,),
        ).fetchone()
        history = db.execute(
            "SELECT before_hash,after_hash FROM federation_merge_history WHERE merge_id=?",
            (merge_id,),
        ).fetchone()
...
def test_identical_merge_replay_noop_and_incompatible_second_merge_rejected(tmp_path):
    ...
    first = reg.merge_entities(
        from_federation_entity_id=a,
        to_federation_entity_id=b,
        decision_id="decision-merge",
    )
    second = reg.merge_entities(
        from_federation_entity_id=a,
        to_federation_entity_id=b,
        decision_id="decision-merge",
    )
...
def test_relationship_redirect_dedupes_without_delete(tmp_path):
    ...
    c = reg.create_entity(entity_type="organization", canonical_name="C")
    _decision(reg, "decision-merge")
    r1 = reg.add_relationship(
        source_federation_entity_id=a,
        target_federation_entity_id=c,
        relationship_type="owns",
        provenance_ids=["p1"],
    )
    r2 = reg.add_relationship(
        source_federation_entity_id=b,
        target_federation_entity_id=c,
        relationship_type="owns",
        provenance_ids=["p2"],
    )
...
def test_supersede_and_tombstone_preserve_history(tmp_path):
    ...
    reg.record_resolution_decision(
        decision_id="sup-1",
        decision_type="superseded_decision",
        reason_code="corrected_identity",
        evidence_ids=["e1"],
        decided_by="reviewer",
    )
...
def test_event_dispositions_fail_closed_and_preserve_rejections(tmp_path):
    ...
    _, bad_hash = reg.record_event(
        event_type="UPSERT",
        source_producer="moneysweep-pr",
        local_record_id="z",
        source_revision="1",
        payload=payload,
        payload_hash_value="0" * 64,
    )
    assert (applied, stale, bad_schema, bad_hash) == (
        "APPLIED",
        "REJECTED_STALE",
        "REJECTED_SCHEMA",
        "REJECTED_HASH",
    ...
    with sqlite3.connect(tmp_path / "hub.db") as db:
        ...
        assert (
            db.execute(
                "SELECT COUNT(*) FROM federation_events WHERE disposition LIKE 'REJECTED_%'"
            ).fetchone()[0]
            == 3
        )
...
def test_non_orderable_revision_requires_monotonic_sequence(tmp_path):
    ...
    _, disposition = reg.record_event(
        event_type="UPSERT",
        source_producer="spiderweb-pr",
        local_record_id="x",
