...
SPEC=importlib.util.spec_from_file_location('guard', Path('scripts/federation_contract_guard.py'))
...
def test_optional_addition_is_minor():
    ...
    b={'type':'object','properties':{'a':{'type':'string'},'b':{'type':'string'}},'required':['a']}
    assert guard.bump_class(a,b)=='MINOR'

def test_new_required_field_is_major():
    a={'type':'object','properties':{'a':{'type':'string'}},'required':['a']}
    b={'type':'object','properties':{'a':{'type':'string'},'b':{'type':'string'}},'required':['a','b']}
    assert guard.bump_class(a,b)=='MAJOR'

def test_property_removal_is_major():
    a={'type':'object','properties':{'a':{'type':'string'},'b':{'type':'string'}}}
...
def test_transitive_closure():
    graph={'edges':[{'from':'centinelas-pr','to':'moneysweep-pr'},{'from':'moneysweep-pr','to':'spiderweb-pr'},{'from':'spiderweb-pr','to':'thehub-pr'}]}
    assert guard.closure(graph,['centinelas-pr']) == ['centinelas-pr','moneysweep-pr','spiderweb-pr','thehub-pr']
