...
- #168 -> `377c2a64eb05ae81a5614b18cb48602316b1c716`; stale Dependabot head `381e4f1d7f4ac45e824d09f6567254e70a7f75a6` was refreshed with current `main`, then repaired with `6a9317381334baeaad9eaf6915c2f7e9b2498c9d` to carry the required federation snapshot freshness update. Local gates passed: clean merge, `git diff --check`, focused fed ledger/remote tests, bounded MAX wrapper assertion with all seven SHAs matching, isolated `tools/requirements-icons.txt` install resolving Pillow `12.3.0`, Pillow import/image smoke, and `tools/build_program_icons.py --repo . --check` in a correctly named disposable `thehub-pr` worktree. All refreshed checks passed including bounded MAX, Hub CI matrix, CodeQL, Semgrep, drift, and gitleaks; the Dependabot head branch was auto-deleted after merge. The icon check emitted a Pillow 14 `Image.getdata` deprecation warning for future cleanup only.
- #170 -> `711d10403e350cfd1310519304da9d49189fdf08`; stale Dependabot head `f1cfad5048f6733ba5b475e04f50613257b59ae5` was refreshed with current `main`, then repaired with `06e3a44b3e900d756b465b05d844f234ee8ca776` to carry the required federation snapshot freshness update. Local gates passed: clean merge, `git diff --check`, template rendering tests, focused fed ledger/remote tests, bounded MAX wrapper assertion with all seven SHAs matching, isolated `requirements-desktop.txt` install, and pywebview `6.2.1` import smoke. All refreshed checks passed including bounded MAX, Hub CI matrix, desktop-build matrix across macOS/Ubuntu/Windows, CodeQL, Semgrep, drift, and gitleaks; the Dependabot head branch was auto-deleted after merge.
- #174 -> `b66fe132a8b8f27d8c720a227d14573415f71a7f`; stale Dependabot head `d95bb3c75de4b65af20d26e4946a9e9fbadc6b52` was refreshed with current `main`, then repaired with `b7d88d3f3450c343fbd8c8cb3063b4a8992462e3` to carry the required federation snapshot freshness update. Local gates passed: clean merge, `git diff --check`, focused fed ledger/remote tests, bounded MAX wrapper assertion with all seven SHAs matching, `npm ci`, frontend lint, frontend tests, frontend build, and jsdom `30.0.1` import/DOM smoke on Node `26.0.0`. `npm ci` reported 13 audit findings for separate vulnerability triage. All refreshed checks passed including bounded MAX, Hub CI matrix, desktop-build matrix across macOS/Ubuntu/Windows, CodeQL, Semgrep, drift, and gitleaks; the Dependabot head branch was auto-deleted after merge.
- #185 -> `3d067035bbd7b3795bc5509bf36928a09bc6aeeb`; Dependabot refreshed head `4f8cd004719b7f56a48040099edaafc135664870` to current `main`, then repaired with `ced01019e839481ad5f8fb078f67e31e5adfd68f` to carry the required federation snapshot freshness update. Local gates passed: already up to date with `origin/main`, `git diff --check`, focused fed ledger/remote tests, bounded MAX wrapper assertion with all seven SHAs matching, `npm ci`, frontend lint, frontend tests, frontend build, and `npm ls` verification for `@testing-library/jest-dom@7.0.1`, `baseline-browser-mapping@2.11.14`, `eslint-plugin-react-refresh@0.5.4`, and `postcss@8.5.26`. `npm ci` reported 13 audit findings for separate vulnerability triage. All refreshed checks passed including bounded MAX, Hub CI matrix, desktop-build matrix across macOS/Ubuntu/Windows, CodeQL, Semgrep, drift, and gitleaks; the Dependabot head branch was auto-deleted after merge.
- #171 -> `2c84e74ca4934bd0fb4fb430c6ec784751dad5fb`; Dependabot refreshed head `ec8176be3b61dfbcf7bb5441713051cee72b8462` to current `main`, then repaired with `06af575867bfa716325a492823dfe26b1ad14dd6` to carry the required federation snapshot freshness update. Local gates passed: already up to date with `origin/main`, `git diff --check`, focused fed ledger/remote tests, bounded MAX wrapper assertion with all seven SHAs matching, `npm ci`, frontend lint, frontend tests, frontend build, `npx tsc --version` reporting `7.0.2`, and `npm ls typescript` resolving `typescript@7.0.2`. `npm ci` reported 13 audit findings for separate vulnerability triage. All refreshed checks passed including bounded MAX, Hub CI matrix, desktop-build matrix across macOS/Ubuntu/Windows, CodeQL, Semgrep, drift, and gitleaks; the Dependabot head branch was auto-deleted after merge.
- #173 -> `6deeacb15e07ea6d7246af9bda82726bea8d1412`; stale Dependabot head `5b7d6d73997b8b66b3c8fab00527b3a8f3a479b0` was refreshed with current `main`, then repaired with `2ebf5cdb570409be463a753ec09cea750d746b52` to carry the required federation snapshot freshness update and complete the React 19 peer set (`react-dom@19.2.8`, `@types/react-dom@19.2.4`, `react-leaflet@5.0.0`, `@react-leaflet/core@3.0.0`). Local gates passed: clean merge, `git diff --check`, focused fed ledger/remote tests, bounded MAX wrapper assertion with all seven SHAs matching, `npm ci`, frontend lint, frontend tests, frontend build, and `npm ls` verification for the React 19 graph. Local Playwright visual was noncanonical on macOS because `tablet/login` failed with the same 25,502-pixel delta on current `origin/main`; hosted `frontend-visual` passed on the repaired head. `npm ci` reported 13 audit findings for separate vulnerability triage. All refreshed checks passed including bounded MAX, Hub CI matrix, desktop-build matrix across macOS/Ubuntu/Windows, CodeQL, Semgrep, drift, and gitleaks; GitHub auto-deleted the Dependabot head branch after merge, and the branch was restored to `2ebf5cdb570409be463a753ec09cea750d746b52`. The post-merge #162 refresh also observed `aguayluz-pr` main advancing from `e9a1736e7dec882ab7e5e67dbcb52de40e655262` to `762e0d50fa6f6416dbbc7cf68f8f556d6fe9497b`, so the bounded registry snapshot was updated to that current head.
- #175 -> `673cdd3326c5d812b222b5bc6630bf208b189fe0`; stale Dependabot head `2fbeeba767684f0a2186b868ce1452681a3a3e68` was refreshed with current `main`, then repaired with `7b324daa2d49521058cf08f18506793be29b801e` to carry the required federation snapshot freshness update and update the canonical `federation-templates/baseline/codeql.yml` alongside the rendered `.github/workflows/codeql.yml` CodeQL Action pins (`ff2f1c621b7f889edc0d3c761ac2e6a3f8cdb0dd`, v4.37.7). Local gates passed: clean merge, template re-render, template drift check, `git diff --check`, 41 focused Python/template/federation tests, bounded MAX wrapper assertion with all seven SHAs matching, `npm ci`, frontend lint, frontend tests, and frontend build. `npm ci` reported 13 audit findings for separate vulnerability triage. All refreshed checks passed including bounded MAX, Hub CI matrix, CodeQL, Semgrep, drift, frontend, frontend-visual, gitleaks, and package checks; GitHub auto-deleted the Dependabot head branch after merge, and the branch was restored to `7b324daa2d49521058cf08f18506793be29b801e`.
- #177 -> `55ff3a2ad763ca5f550cc698617ff2e268c4b672`; Dependabot refreshed head `01f39f0c1c0bc25724f1e5454d516c868536914c` to current `main`, then repaired with `45cafc2a36df8082fe3e4bdbf99a19ed84860aa8` to carry the required federation snapshot freshness update and update canonical `federation-templates/baseline/pip-audit.yml` alongside the rendered checkout v7 workflow pins. Local gates passed: already up to date with `origin/main`, template re-render, template drift check, `git diff --check`, 41 focused Python/template/federation tests, bounded MAX wrapper assertion with all seven SHAs matching, `npm ci`, frontend lint, frontend tests, and frontend build. `npm ci` reported 13 audit findings for separate vulnerability triage. All refreshed checks passed including bounded MAX, Hub CI matrix, desktop-build matrix across macOS/Ubuntu/Windows, CodeQL, Semgrep, drift, frontend, frontend-visual, federation-design verify-and-pack, validation workflows, and gitleaks; GitHub auto-deleted the Dependabot head branch after merge, and the branch was restored to `45cafc2a36df8082fe3e4bdbf99a19ed84860aa8`.
- #176 -> `94d2c0d53f2d99e8b5c248e3279921c4964a59f5`; Dependabot refreshed head `14870e0fee64ca6fecf85ce4a47cea54beb833ad` to current `main`, then repaired with `67e858dd19f51616f54b7be30247e1d9255cc20b` to carry the required federation snapshot freshness update and update canonical `federation-templates/baseline/pip-audit.yml` alongside rendered setup-python v7 workflow pins (`5fda3b95a4ea91299a34e894583c3862153e4b97`, v7.0.0). Local gates passed: already up to date with `origin/main`, template re-render, template drift check, `git diff --check`, 41 focused Python/template/federation tests, bounded MAX wrapper assertion with all seven SHAs matching, `npm ci`, frontend lint, frontend tests, and frontend build. `npm ci` reported 13 audit findings for separate vulnerability triage. All refreshed checks passed including bounded MAX, Hub CI matrix, desktop-build matrix across macOS/Ubuntu/Windows, CodeQL, Semgrep, drift, frontend, frontend-visual, validation workflows, and gitleaks; GitHub auto-deleted the Dependabot head branch after merge, and the branch was restored to `67e858dd19f51616f54b7be30247e1d9255cc20b`. The post-merge #162 refresh also observed `aguayluz-pr` main advancing from `762e0d50fa6f6416dbbc7cf68f8f556d6fe9497b` to `1da22589f0b532880e0e7968219d8ebf7e10283a`, so the bounded registry snapshot was updated to that current head.
- #179 -> `7f674026992cd36a3fb9a9f4c218bfd19ac6288a`; Dependabot refreshed the PR title/scope to `astral-sh/setup-uv` `10.0.1` and head `a8e4cffd4231985f865c772f9db4d5ded97a3201`, then the branch was repaired with `4741bfc64649d2bd5b56975f201042cbe53dd046` to merge current `main`, update canonical `federation-templates/baseline/pip-audit.yml`, re-render `.github/workflows/pip-audit.yml`, and refresh the bounded federation snapshot. Local gates passed: template drift check, `git diff --check`, 41 focused Python/template/federation tests, bounded MAX wrapper assertion with all seven SHAs matching, `uv lock --check`, `npm ci`, frontend lint, frontend tests, and frontend build. `npm ci` reported 13 audit findings for separate vulnerability triage. The first hosted `frontend-visual` job was cancelled after six hours while installing Playwright system dependencies and never reached visual tests; rerunning failed jobs on the same `4741bfc64649d2bd5b56975f201042cbe53dd046` head passed `frontend-visual` in 1m15s. All refreshed checks then passed including bounded MAX, Hub CI matrix, CodeQL, Semgrep, drift, frontend, frontend-visual, validation workflows, and gitleaks; GitHub auto-deleted the Dependabot head branch after merge, and the branch was restored to `4741bfc64649d2bd5b56975f201042cbe53dd046`. The post-merge #162 refresh also observed current main-head drift across the six producer repositories, so the bounded registry snapshot was updated to the live August 22 heads without certifying downstream OPEN or BLOCKED vectors as complete.
- #190 -> `7d7b8d80b0d2ca7656e7faa707ee04d236f4c192`; non-draft GUI parity rollup PR was stale-green on base `4611327f5f1503fafd7ae2a03edffdb636cedc27`, then refreshed onto current `main` with `cbb4f13a2d6f9d5fc628698d70a4594e3c2a2f7c`, repaired in `35eb12f8ea3038ea262f1d9d6340c0dd105d938f` to keep bounded MAX asserting the current-snapshot invariant instead of requiring intentionally stale SHAs, and refreshed again with `b36513e38a66e0eb49abde991dcedde7657fd35a` after `ovnis-pr` advanced during hosted validation. Local gates passed: template drift check, `git diff --check`, 61 focused GUI parity/template/federation tests, and bounded MAX wrapper assertion with all seven SHAs matching. Fresh hosted checks passed on `b36513e38a66e0eb49abde991dcedde7657fd35a`, including bounded MAX, Hub CI matrix, CodeQL, Semgrep, drift, frontend, frontend-visual, validation workflows, and gitleaks; GitHub auto-deleted the PR head branch after merge, and the branch was restored to `b36513e38a66e0eb49abde991dcedde7657fd35a`. The post-merge #162 refresh updated TheHub to `7d7b8d80b0d2ca7656e7faa707ee04d236f4c192` and retained the live August 22 producer heads without certifying downstream OPEN or BLOCKED vectors as complete.
- #163 -> `f06799fbdb478531622a85db248ec26e83fb5f03`; non-draft ontology canon PR was stale-green on base `a5bad05ce5eec4cc73e2eed8569d294bc3f5cd27`, then refreshed onto current `main` with merge commit `d615146` and repaired in `c3a10dbdedd3ac1653d6ee95e068920da91c64cc` to align the new ontology workflow with current action pins, fix ontology-tool lint issues, and refresh the bounded federation snapshot to live heads (`thehub-pr` `7d7b8d80b0d2ca7656e7faa707ee04d236f4c192`, `ovnis-pr` `aa86cbd04bc0ce47593277bc95d5f34ca37e4acd`). Local gates passed: ontology manifest check, ontology validator, template drift check, `git diff --check`, 49 focused ontology/template/federation tests, bounded MAX wrapper assertion with all seven SHAs matching, `ruff`, `mypy`, `uv lock --check`, and full pytest (`985 passed, 3 skipped`) after installing the worktree editably to repair the local `hub` console-script environment. Fresh hosted checks passed on `c3a10dbdedd3ac1653d6ee95e068920da91c64cc`, including two `exhaustive-concordance` runs, bounded MAX, Hub CI matrix, CodeQL, Semgrep, drift, frontend, frontend-visual, and gitleaks; `audit-claims-federation` was the expected skip. The ontology repository pins remain the declared 2026-08-05 semantic snapshot rather than a silently repinned source universe. GitHub auto-deleted the PR head branch after merge, and the branch was restored to `c3a10dbdedd3ac1653d6ee95e068920da91c64cc`. The post-merge #162 refresh updated TheHub to `f06799fbdb478531622a85db248ec26e83fb5f03` without certifying downstream OPEN or BLOCKED vectors as complete.

## Dependency Closures Completed In This Pass

- #172 | INVALID_DEPENDENCY_CLOSE | Closed 2026-08-17 because refreshed current-head checks failed in `npm ci` with `ERESOLVE`: `eslint@10.8.1` conflicts with `eslint-plugin-react@7.37.5`, whose latest npm peer range remains `eslint "^3 || ^4 || ^5 || ^6 || ^7 || ^8 || ^9.7"`. `eslint-plugin-react-refresh@0.5.4` and `eslint-plugin-unused-imports@4.4.1` already allow ESLint 10, so the blocker is specifically the React plugin ecosystem. Recreate only after `eslint-plugin-react` supports ESLint 10 or the project intentionally replaces/removes that plugin. Branch `dependabot/npm_and_yarn/server/frontend/eslint-10.8.0` remained present at `6d794ebab2120a165d2911ebe9a63956dc822d86` after closure.
...
- `moneysweep-pr` #471 -> `c9d7b603939104679e6ffb913aae15e807dcfa1d`; stale Dependabot head `6418f45638bc93372042b576f5aad4f7b1390ef5` was refreshed with current `main` after #478, then repaired with `374a861f5f584094cd8d54c03816dfe550770013` so `requirements.txt` remained the documented lower-bound install file while `requirements.in` and `requirements.lock` carried `pandas>=3.0.5` / `pandas==3.0.5`. Local gates passed: clean merge, `requirements.in == requirements.txt`, lock content assertion, `git diff --check`, and `pip install -r requirements.txt` in a disposable venv; all 33 refreshed checks passed including `lock-sync`, required review was approved, and the head branch was auto-deleted after merge.
- `moneysweep-pr` #479 -> `dc234ed46ac71302e8fcc675df70bc23bbdc3e63`; stale Dependabot head `5888942d2854badc2f1b90aba317c4faf8251b1b` was refreshed with current `main` after #471 into `8ed5a1f6409994df25d82181bbf25e57457b64df`. Local dashboard gates passed: clean merge, `git diff --check`, `npm ci`, `npm run lint`, `npm test` (3 files / 50 tests), and `npm run build`; `npm ci` reported 7 audit findings for separate vulnerability triage. All 34 refreshed checks passed, required review was approved, and the head branch was auto-deleted after merge.
...
`moneysweep-pr` dependency queue exhausted on 2026-08-17 after REST returned no open `dependabot/*` pull requests; #473 and #474 were closed fail-closed with branch preservation, and #478, #471, and #479 were repaired, approved, green, and merged.
